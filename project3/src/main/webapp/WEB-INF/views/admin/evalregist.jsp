...
<div class="row">
    ...
    <div class="col-md-7" style="margin-left: -200px;">
        <h2 class="text-center">수강평 작성</h2>
 		<form action="<%=request.getContextPath() %>/admin/evalregist" onsubmit="return false;"name="evalForm" method="post">
 			<input type="hidden" id="id" name="id" value="${user.id}"/>
						<div class="form-row">
							<div class="form-group col-sm-6">
								<label>강의명</label>
								<input type="text" name="lecturename" class="form-control" maxlength="20" id="lecturename"/>
							</div>
							<div class="form-group col-sm-6">
								<label>교수명</label>
								<input type="text" name="professorname" class="form-control" maxlength="20" id="professorname"/>
							</div>
						</div>
						<div class="form-row">
...
$(function(){
	...
	$('#regist').on('click',function(){
		...
		$.ajax({
			...
			,success:function(res){
				alert('글을 작성하였습니다');
				window.opener.document.location.href="<%=request.getContextPath()%>/admin/evalList"; 
				self.close();
			},error:function(req){
				alert('상태:'+req.status);
			},dataType:'json'
	...
	$('#list').on('click',function(){
		location.href="<%=request.getContextPath()%>/admin/evalList"
	})
})
