		...
		<div id="cal_bg" class="card shadow mb-4">
			<div class="card-header d-flex justify-content-between align-items-center" style = "text-align: center;">
				<h6 class="text-primary font-weight-bold m-0">
					<span style="color: #46546e; font-weight: bold; text-align: center;"><span id="postTit" style = "font-size: 25px; margin-left: 5px;">${freeBoard.title}</span></span>
				</h6>
			</div>
			<div class="card-body">
				<div class="cal-area" id="calarea">
					...
					<div class = "row">
						<div class = "col-lg-6">
							<span id = "id" style="font-size: 18px; font-weight: bolder;">${freeBoard.name}</span>
							<span id = "ctrgb">(${freeBoard.regdate})</span>
						</div>	
					...
					<div class = "row">
						...
						<div class = "col-lg-4" style = "text-align: right;">
							<div id = "originShowbtn">
							   <c:if test="${freeBoard.ref_step <=8}">
							   	<form id="answerForm" action="<%=request.getContextPath()%>/freeBoard/answerForm" method="post" style="float:right;">
									<input type="hidden" name="bno" id="bno" value="${freeBoard.bno}"/>
									<input type="button" class="btn btn-primary" value="답변" id="answer">
								</form>	
							   </c:if>
								<form id="modifyForm" action="<%=request.getContextPath()%>/freeBoard/modifyForm" method="post" style="float:right;margin: 0 5px;">
									<input type="hidden" name="bno" id="bno" value="${freeBoard.bno}"/>
									<input type="button" class="btn btn-warning" value="수정" id="modify">
								</form>
								<form id="deleteForm" action="<%=request.getContextPath()%>/freeBoard/Delete" method="post" style="float:right;">
									<input type="hidden" name="bno" id="bno" value="${freeBoard.bno}"/>
